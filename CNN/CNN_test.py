...
for phase in ['test']:
    ...
    for i, batch in enumerate(loader):
        ...
        # rexcel = open_workbook("data_2class.xls")  # 用wlrd提供的方法读取一个excel文件
        # rows = rexcel.sheets()[0].nrows  # 用wlrd提供的方法获得现在已有的行数
        # excel = copy(rexcel)  # 用xlutils提供的copy方法将xlrd的对象转化为xlwt的对象
        # table = excel.get_sheet(0)  # 用xlwt对象的方法获得要操作的sheet
        # table.write(i, 0, str(labels.data.cpu().numpy()[0]))  # xlwt对象的写方法，参数分别是行、列、值
        # table.write(i, 1, str(myfeature[0, :].tolist()))  # xlwt对象的写方法，参数分别是行、列、值
        # excel.save("data_2class.xls")  # xlwt对象的保存方法，这时便覆盖掉了原来的excel
        # # table.close()
        # # excel.close()
        #
        # rexcel = open_workbook("predict_2class.xls")  # 用wlrd提供的方法读取一个excel文件
        # rows = rexcel.sheets()[0].nrows  # 用wlrd提供的方法获得现在已有的行数
        # excel = copy(rexcel)  # 用xlutils提供的copy方法将xlrd的对象转化为xlwt的对象
        # table = excel.get_sheet(0)  # 用xlwt对象的方法获得要操作的sheet
        # table.write(i, 0, str(labels.data.cpu().numpy()[0]))  # xlwt对象的写方法，参数分别是行、列、值
        # table.write(i, 1, str(predict.data.cpu().numpy()[0]))  # xlwt对象的写方法，参数分别是行、列、值
        # excel.save("predict_2class.xls")  # xlwt对象的保存方法，这时便覆盖掉了原来的excel
        # table.close()
        # excel.close()

rexcel = open_workbook("result_2class.xls")  # 用wlrd提供的方法读取一个excel文件
rows = rexcel.sheets()[0].nrows  # 用wlrd提供的方法获得现在已有的行数
excel = copy(rexcel)  # 用xlutils提供的copy方法将xlrd的对象转化为xlwt的对象
table = excel.get_sheet(0)  # 用xlwt对象的方法获得要操作的sheet
table.write(0, 0, str(label_list))  # xlwt对象的写方法，参数分别是行、列、值
table.write(1, 0, str(prediction_list))  # xlwt对象的写方法，参数分别是行、列、值
excel.save("result_2class.xls")  # xlwt对象的保存方法，这时便覆盖掉了原来的excel